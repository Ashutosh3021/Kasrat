  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex justify-center animate-fadeIn">
      <div className="w-full max-w-[390px] min-h-screen relative pb-8 flex flex-col bg-black">
        <header className="flex items-center justify-between h-14 px-4 w-full bg-black border-b border-[#2C2C2E] sticky top-0 z-50">
        ...
        <main className="flex-1 px-4 pt-8 flex flex-col gap-4">
          ...
          {RELEASES.map(r => (
            <article
              key={r.version}
              className={`bg-[#1C1C1E] rounded-lg p-3 relative overflow-hidden ${r.isNew ? 'border-l-4 border-[#3B82F6]' : 'border border-[#2C2C2E]'}`}
            >
              {r.isNew && (
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                {r.isNew && (
                  <span className="bg-[#2C2C2E] text-white text-[13px] font-medium px-3 py-1 rounded-full flex items-center gap-1 border border-[#2C2C2E]">
                    New 🎉
                  </span>
                )}
              </div>
              <ul className="flex flex-col gap-3 relative z-10">
                {r.changes.map((c, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <p className="text-[17px] text-[#c2c6d6]">{c}</p>
                  </li>
                ))}
              </ul>
            </article>
          ...
          <div className="mt-8 pt-4 border-t border-[#2C2C2E]">
            <button
              onClick={onClose}
              className="w-full bg-transparent border border-[#2C2C2E] text-white font-semibold text-[15px] h-12 rounded-lg flex items-center justify-center gap-2 hover:bg-[#2C2C2E] transition-colors"
            >
              Close
            </button>
          </div>
