    ...
    <div className="min-h-screen bg-black pb-24 pt-14">
      ...
      <main className="px-4 pt-6 max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-[32px] font-bold leading-10 tracking-tight text-white">History</h1>
        </div>
        ...
        <div className="flex gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#1C1C1E] border border-[#2C2C2E] text-white rounded-lg pl-10 pr-4 py-3 text-[17px] focus:border-[#3B82F6] focus:outline-none placeholder:text-zinc-500"
              placeholder="Search workouts..."
            />
          </div>
          <button
            onClick={() => setFilterOpen(true)}
            className="bg-[#1C1C1E] border border-[#2C2C2E] p-3 rounded-lg text-zinc-400 hover:text-white"
          >
            <Filter size={20} />
          </button>
        </div>
        ...
        {Object.keys(grouped).length === 0 ? (
          <div className="text-center py-16">
            <p className="text-[#c6c6cb] text-[15px]">No history yet. Start logging sets!</p>
          </div>
        ) : (
          <div className="space-y-8">
            {Object.entries(grouped).map(([date, items]) => (
              <section key={date}>
                <h2 className="text-[22px] font-semibold text-white mb-4 border-b border-[#2C2C2E] pb-2">{date}</h2>
