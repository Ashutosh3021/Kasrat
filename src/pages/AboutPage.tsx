...
export default function AboutPage() {
  ...
  return (
    <div className="min-h-screen bg-[#151515] pb-8">
      <nav className="fixed top-0 w-full z-50 flex items-center justify-between px-3 h-14 bg-[#151515] border-b border-[#2C2C2E]">
        <button onClick={() => navigate(-1)} className="text-zinc-400 hover:opacity-80 p-2 -ml-2">
          <ArrowLeft size={22} strokeWidth={1.5} />
        </button>
        <span className="text-xl font-black italic text-[#93032E] tracking-tight uppercase font-bold">KASRAT</span>
      ...
      <main className="pt-24 px-3 pb-[env(safe-area-inset-bottom)] flex flex-col items-center max-w-lg mx-auto w-full">
        <header className="flex flex-col items-center mb-8 w-full">
          <div className="w-28 h-28 bg-[#93032E] rounded-[4px] flex items-center justify-center mb-6">
          ...
          <p className="text-[13px] font-medium text-[#A1A1A6] tracking-widest uppercase mb-3">Version 1.2.36</p>
        ...
        <nav aria-label="About App Links" className="w-full flex flex-col gap-3 mb-8">
          {LINKS.map(link => (
            <a
              ...
              className="group bg-[#1C1C1E] rounded-[4px] p-3 flex items-center justify-between active:scale-[0.98] transition-transform border border-[#2C2C2E] focus:border-[#93032E] outline-none"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-[2px] bg-[#2C2C2E] flex items-center justify-center text-[#A1A1A6] group-hover:text-[#93032E] transition-colors">
        ...
        <footer className="mt-auto pt-8 pb-8 flex flex-col items-center w-full">
          ...
          <p className="text-[13px] font-medium text-[#424754] mt-2">© 2026 KASRAT</p>
        </footer>
      </main>
    </div>
  )
