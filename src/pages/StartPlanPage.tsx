...
import { ArrowLeft, Plus, CheckCircle, Lock, MoreHorizontal, Camera, FileText } from 'lucide-react'
import { db, type Plan, type PlanExercise, type GymSet } from '../db/database'
import { useTimerStore } from '../store/timerStore'
import { useSettingsStore } from '../store/settingsStore'

interface LoggedSet { weight: number; reps: number }
...
export default function StartPlanPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!p) return
      setPlan(p)
      const exs = await db.plan_exercises.where('planId').equals(Number(id)).filter(e => e.enabled).toArray()
      setExercises(exs)
    }
    load()
  }, [id])
  ...
  const totalDone = completedExercises.size
  ...
  async function logSet() {
    ...
    const key = currentEx.exercise
    setLoggedSets(prev => ({ ...prev, [key]: [...(prev[key] ?? []), { weight: parseFloat(weight), reps: parseInt(reps) }] }))
    setReps('')
    if (settings.restTimers) startTimer(settings.timerDuration)
    const newSets = [...(loggedSets[key] ?? []), { weight: parseFloat(weight), reps: parseInt(reps) }]
    if (newSets.length >= currentEx.maxSets) {
      setCompletedExercises(prev => new Set([...prev, key]))
    }
  }

  function finishExercise() {
    setCompletedExercises(prev => new Set([...prev, currentEx.exercise]))
    ...
    <div className="min-h-screen bg-black pb-8">
      <header className="fixed top-0 w-full z-40 bg-black/90 backdrop-blur-md flex flex-col">
        ...
        <div className="px-4 py-2 text-center bg-black">
          <span className="text-[13px] font-medium text-[#A1A1A6] uppercase tracking-widest">{totalDone}/{exercises.length} exercises done</span>
        </div>
      </header>

      <main className="flex-1 px-4 pt-28 pb-8 flex flex-col gap-4">
        {exercises.map((ex, i) => {
          const done = completedExercises.has(ex.exercise)
            ...
            <article key={ex.id} className="bg-[#1C1C1E] rounded-lg p-4 flex flex-col gap-4 relative overflow-hidden border border-[#3B82F6] shadow-[0_0_15px_rgba(59,130,246,0.15)]">
                ...
                {sets.map((s, si) => (
                  <div key={si} className="flex items-center justify-between py-2 border-b border-[#2C2C2E]">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[17px] text-white font-semibold">{s.weight} kg</span>
              ...
              <div className="mt-2 bg-black/50 p-4 rounded-lg border border-[#2C2C2E] flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex gap-3">
                    <div className="relative flex-1">
                      <label className="absolute -top-2 left-3 bg-[#131315] px-1 text-[10px] text-[#A1A1A6]">Weight ({settings.strengthUnit})</label>
                      <input
                        type="number"
                        value={weight}
                ...
                <div className="flex gap-3">
                  <button onClick={logSet} className="flex-1 bg-[#3B82F6] text-white font-semibold text-[15px] py-3 rounded-lg flex items-center justify-center gap-2 h-12">
                    <Plus size={18} />
                    Log Set
                  </button>
              ...
              <button onClick={finishExercise} className="w-full mt-2 border border-[#2C2C2E] text-white font-semibold text-[15px] py-3 rounded-lg bg-transparent hover:bg-[#2C2C2E] transition-colors h-12">
                Finish Exercise
              </button>
            </article>
          )
